...
## Planopt M&S configurations
def _get_merge_and_shrink_config(shrink: str, label_reduction: bool, prune: bool):
    ms = "merge_strategy=merge_precomputed(linear())"

    if shrink == "f-preserving":
        ms += ",shrink_strategy=shrink_fh()"
    elif shrink == "bisimulation":
        ms += ",shrink_strategy=shrink_bisimulation()"
    else:
        raise NotImplementedError

    if label_reduction:
        ms += ",label_reduction=exact(true,true)"

    ms += f",prune_irrelevant_states={prune}"

    return ["--search", f"astar(merge_and_shrink({ms}))"]


ALIASES["ms-f-pres"] = _get_merge_and_shrink_config("f-preserving", False, True)
ALIASES["ms-f-pres-label"] = _get_merge_and_shrink_config("f-preserving", True, True)
ALIASES["ms-bisim"] = _get_merge_and_shrink_config("bisimulation", False, True)
ALIASES["ms-bisim-label"] = _get_merge_and_shrink_config("bisimulation", True, True)
